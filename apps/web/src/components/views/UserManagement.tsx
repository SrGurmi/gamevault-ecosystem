        ...
        (loading ? (
          ...
          <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
            {items.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-3 p-3 rounded-xl bg-white/3 border border-indigo-500/20 mb-4"
              >
                <img
    ...
    <div className="flex items-center p-4 rounded-2xl bg-white/3 border border-white/5 hover:border-emerald-500/20 transition-all gap-4">
      <div className="flex items-center gap-3 mb-4">
        <div className="relative shrink-0">
          <img
            src={
              user.avatar_url || "https://placehold.co/150/0c1628/10b981?text=?"
            }
        ...
        <div className="flex-1 min-w-0">
          ...
          <span
            className={`text-[9px] font-black uppercase tracking-widest ${user.role === "admin" ? "text-emerald-400" : "text-slate-500"}`}
          >
            {user.role}
          </span>
          {user.updated_at && (
            <p className="text-[9px] text-slate-600 mt-0.5">
              Modificado {timeAgo(user.updated_at)}
            </p>
          )}
        </div>
      </div>
