    ...
    return (
      <div className="py-32 flex flex-col items-center text-slate-600">
        ...
        <p className="text-sm mt-1">
          Escanea juegos con la app móvil para poblar la colección
        </p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5">
        {items.map((item, i) => (
          <div
            ...
            style={{
              animation: `fadeInUp 0.4s ease ${Math.min(i * 0.04, 0.6)}s both`,
            }}
            ...
            <div className="relative aspect-3/4 rounded-xl overflow-hidden mb-3 bg-black/40 group-hover:shadow-[0_0_20px_rgba(16,185,129,0.3)] transition-all">
              ...
              {item.profiles && (
                <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1.5 bg-black/70 backdrop-blur-sm px-2 py-1 rounded-full border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <img
                    src={item.profiles.avatar_url || "https://placehold.co/24"}
                    className="w-4 h-4 rounded-full"
                    alt=""
                  />
                  <span className="text-[8px] font-bold text-slate-300 max-w-[60px] truncate">
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div
          ...
          style={{
            animation: `fadeInUp 0.3s ease ${Math.min(i * 0.03, 0.5)}s both`,
          }}
        >
          <img
            src={img(item.games?.cover_url)}
            className="w-12 h-16 object-cover rounded-lg shrink-0 grayscale group-hover:grayscale-0 transition-all"
            alt=""
            loading="lazy"
          />
          ...
          <span className="text-xs text-slate-600 hidden lg:block w-16 text-right">
            {timeAgo(item.created_at)}
          </span>
          <Icon
            name="chevron"
            className="w-4 h-4 text-slate-700 group-hover:text-emerald-500 transition-colors shrink-0"
          />
        </div>
      ))}
    </div>
